...
- **BREAKING CHANGE:** changed the pipeline success rate — on the contributors table and on the Insights tile — to divide by the runs that reached a verdict rather than by every run, and added `pipelineRunsFailed` to `ContributorSummary` beside `pipelineRunsSucceeded`. The field is required, so anything that constructs a `ContributorSummary` — a test fixture, a stub backend, a second frontend — has to supply it, which is the same shape of change the `identities` field was. A run cancelled because a newer push superseded it, skipped by a path filter, or still in progress is neither a success nor a failure, and under a workflow that cancels in-progress runs it was most of the denominator, so a busy afternoon read as a bad success rate
- **BREAKING CHANGE:** changed the upgrade to re-walk every tracked repository's history once. The rows stored before the attribution fix credit the merger and cannot be repaired in place, because the facts the fix needs — parent counts, merge commits, the commit a run built — were never stored. The new migration sends each tracked repository's ingestion cursors back to where a fresh install starts, forgets the days it claimed as fetched, and removes the commits, pull requests, reviews and runs the walk re-collects; releases and tags come from the daily snapshot and stay, and a repository that has left the catalog keeps its history because nothing would ever put it back. A host application that picks this release up unattended loses its stored history until the backfill catches up: the dashboard behaves as it does after installation, with the last day answerable from the first run and wider ranges unlocking as the backfill advances
- changed both `chlog new` examples in the AI-assistant instruction block of `CLAUDE.md` and `.github/copilot-instructions.md` to `--body '<past-tense description>'`: changelog bodies here are written in simple past tense, and the body is single-quoted because it carries backticks that a double-quoted shell argument would command-substitute, and added the line telling the reader to write an apostrophe inside the single-quoted body as `'\''`, since bodies here carry possessives, and switched the 5 other hand-written `chlog new` examples in `.github/copilot-instructions.md`, `CONTRIBUTING.md`, `.github/pull_request_template/default.md`, and `.github/skills/code-review/SKILL.md` to the same single-quoted body argument
...
- changed the Sonar figures on a contributor row to sum over the repositories the person committed to or merged a pull request into, rather than every repository any event of theirs landed in. Sonar measures a project, not a person; reviewing a repository's pull requests or triggering its pipeline does not put a hand on its code, and counting either put every repository's bugs and debt on the row of whoever reviews the most — usually the person who also merges the most. Every Sonar heading on the table now says what the figure is
- changed what counts as a review: a vote cast on somebody else's pull request. The author's own vote no longer counts, on either provider, because it is not a review and counting it let an author pad their figures by replying to their reviewers; and on Azure DevOps a reviewer who was added and never voted no longer counts either, because that API lists everyone a policy or a person added and most of them never look
...
- refreshed `CLAUDE.md` to list the `20260825000000_catalog_facts.js` migration in the backend architecture table, which had documented only the `init` and `identities` migrations even though the catalog-derived columns it adds are the ones that back the documentation and API grades described in the same file
...
- fixed merged work being credited to whoever merged it rather than to whoever did it. Both providers stamp the merger on everything a merge produces — the squash commit Azure DevOps authors as whoever pressed *Complete*, the merge commit GitHub authors as the merger and loads with the whole pull request's diff, and the post-merge pipeline run both report as requested by the merger — so on a team where one or two people complete most pull requests, the contributors table showed them as the authors of everything: the commits, the churn, the pipeline runs, and through those the Sonar bugs and debt of every repository they merged into. `attributeMergedWork` now corrects all of it in one place, fed by both collectors: a squash commit is credited to the pull request's author; a merge commit is not counted at all, whether a pull request produced it or a `git merge` did, because its diff is the sum of the commits it joins; a rebase is left alone; and a pipeline run belongs to the author of the change it built — the pull request whose merge produced the commit, else the commit's author, else whoever requested it. The commits a merge commit brought in are fetched from the pull request and stored under the dates they were written, because they never appear in the branch history for the day of the merge and the day they were written was fetched before they were on the branch; without that, dropping the merge commit would make the work vanish from everybody's row. GitHub reports the parent count on every commit; Azure DevOps reports it on no list endpoint, so a merge commit no pull request names is recognised by the message forms git writes itself, and the pull request's `completionOptions` decides between a squash, a rebase and a plain merge. The rules are stated in full in `plugins/code-health-backend/docs/attribution.md`
- fixed the display name of an Azure DevOps commit author, which was read from the identity-service shape and fell through to the e-mail address: git's own metadata on a commit carries `name` and `email`, and the `name` is now read
...
- raised `js-yaml` to `3.15.2` on the 3.x line and `4.3.2` on the 4.x line, and `svgo` to `2.8.4`, all through `resolutions`, to clear the three advisories `sca:yarn-audit` began failing on: GHSA-2883-xcg3-v3hh in `js-yaml`, reached through `yml-loader` on the 3.x line and `@backstage/core-components` on the 4.x line, and GHSA-w27v-7q3p-w38r in `svgo`, reached through `postcss-svgo`. The existing `js-yaml` resolutions pinned `^4.3.1`, one release below the fix, and the 3.x line had none. Every target was published more than a week ago, so `npmMinimalAgeGate: '7d'` lets it through
