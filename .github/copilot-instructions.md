...
- **The bump desynchronises `yarn.lock`.** `.autobump.yaml` moves the caret range the frontend and
  ...
  CI job's `yarn install --immutable` answers `YN0028` until the lockfile is regenerated. This
  repository's own `.autobump.yaml` sets `refresh: true`, which makes AutoBump regenerate it inside
  the bump commit. Requires AutoBump 3.0.0, which replaced the old `refresh_commands` argv — an
  executable read from configuration, and therefore never honoured from a repository's own file —
  with a flag and built-in recipes. See `CLAUDE.md` > Release.
