---
name: code-review
description: "Review pull requests and diffs in backstage-plugin-code-health — the Backstage frontend and backend plugin showing CI, release and compliance metrics — against the rios0rios0/guide standards: hexagonal layering, `snake_case` files, no `any`, hand-rolled doubles, and the 95 % coverage floor. Use when reviewing a PR, a branch, or staged changes here."
---

# Code review — `backstage-plugin-code-health`

This repository holds three packages — `code-health` (frontend), `code-health-backend`, and `code-health-common` — that surface CI status, releases, tags, compliance checks, and contributor metrics for every GitHub and Azure DevOps repository in a Backstage catalogue. It talks to provider APIs with real tokens and stores results in a database.
...
| Frontend Design — the five layers | [Frontend-Design](https://github.com/rios0rios0/guide/wiki/Frontend-Design) |
...
## What matters most in `backstage-plugin-code-health`
...
plugins/code-health-backend/src/
  domain/entities|commands|repositories|services/     ports and business rules
  infrastructure/repositories|services|http|controllers/
  plugin.ts
plugins/code-health/src/
  domain/ service/ infrastructure/ presentation/ main/   the five frontend layers
plugins/code-health-common/                              shared types
```

- **The backend is hexagonal and the frontend has five layers.** `domain/` holds entities, commands, and ports; `infrastructure/` holds implementations; the arrow points inward. On the frontend, `presentation/` never imports an infrastructure module directly — `main/` wires them. A violation is a Critical finding.
- **Never run `jest`, `eslint`, `gitleaks`, `semgrep`, `trivy`, `hadolint`, or `codeql` directly.** Use the `make` targets, which load the [pipelines](https://github.com/rios0rios0/pipelines) configuration first.
- **Root scripts use `backstage-cli repo lint | test | build`**, not the per-package commands, because `repo test --coverage` writes the merged `coverage/` and `junit-report.xml` at the repository root where the shared pipeline reads them. A change that reverts to per-package scripts breaks the pipeline's reporting.
- **Coverage is enforced repo-wide at 95 % lines/statements, 92 % functions, 88 % branches.** Write the test rather than adding an exclusion; a new coverage exclusion is a Critical finding unless it is argued in the pull-request description.
...
- **Merged work is credited to whoever did it, never to whoever merged it.** `attributeMergedWork` in `domain/entities/merge_attribution.ts` is the single place the correction lives, and both collectors must feed every commit and build through it. A collector that stores the provider's stamp on a squash commit, counts a merge commit, or credits a post-merge run to whoever requested it reintroduces the bug the rule exists to fix — these figures are read as a measure of people, so treat it as a Critical finding. A review is a vote on somebody else's pull request; the pipeline success rate divides by decided runs; Sonar on a contributor row sums over repositories committed to or merged into.
...
```bash
yarn typecheck        # one tsc pass over the workspace   (~25 s)
make lint             # ESLint + knip across all packages (~45 s)
make test             # Jest across all packages          (~45 s)
yarn build            # tsc then backstage-cli repo build (~45 s)
...
- The store is tested against a real database (`TestDatabases`) with the real migrations applied — never against a SQL mock.
- Collectors and the HTTP gateway are tested against a real `http.createServer`; the plugin is tested through `startTestBackend` with `supertest`.
- Ingestion stays behind a manual trigger in the plugin tests so they never reach the network. A test that would make a real API call is a Critical finding.
- Doubles are hand-rolled in `test/doubles/`, builders in `test/builders/` — no mock library, and no `jest.mock()` module mocking.
...
- `node_modules/`, `dist-types/`, `coverage/`, and `junit-report.xml` appearing in the tree — they are build output. Do flag a commit that actually adds them to git.
- Material UI v4 patterns that look dated; the version is pinned by Backstage.
